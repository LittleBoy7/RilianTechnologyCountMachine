...
class SAModule_tail(nn.Module):
    def __init__(self, in_channels, out_channels, use_bn):
        super(SAModule_tail, self).__init__()
        branch_out = out_channels // 4
        self.branch9x9 = BasicConv(in_channels, branch_out, use_bn=use_bn,
    ...
    def forward(self, x):
        branch9x9 = self.branch9x9(x)
        branch3x3 = self.branch3x3(x)
        branch5x5 = self.branch5x5(x)
        branch7x7 = self.branch7x7(x)
        out = torch.cat([branch9x9, branch3x3, branch5x5, branch7x7], 1)
        return out
